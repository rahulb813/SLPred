...
from load_saved_models import form_dict_saved_models, read_threshold_weights
from utils import (read_data, read_iFeatures, read_pssm_features,
                   read_spmap_features)
from utils_train_data import (read_data_train, read_pssm_features_train,
                              read_spmap_features_train)
...
def read_test_data(feature_directory, file_name, loc, feature_name):
    ...
    if feature_name in set_iFeatures:
        ...
        test_prot_id_list, X_test = \
            read_spmap_features(feature_directory,
                                file_name, loc, feature_name)
    else:
        X_test = read_pssm_features(
            feature_directory,  file_name, feature_name)
    # print(X_test)
...
def form_probabilistic_results(feature_directory, file_name, loc,  dict_feature_norm_models, dict_threshold_weights):
    ...
    for feature_norm in dict_feature_norm_models:
        ...
        test_prot_id_list, X_test = read_test_data(
            feature_directory, file_name, loc, feature_name)
...
def predict_sub_locs(feature_directory, file_name):
    ...
    for loc in dict_locs_feature_norm_models:
        all_preds, list_weights = form_probabilistic_results(
            feature_directory, file_name, loc, dict_locs_feature_norm_models[loc], dict_locs_threshold_weights[loc])
...
def write_predictions_to_a_file(file_name, dict_prot_id_loc_predictions):
    ...
    with open("predictions/{}_predictions.csv".format(file_name),  "w") as fw:
        ...
        for prot_id in dict_prot_id_loc_predictions:
            ...
            for loc_name in loc_list:
                fw.write(
                    "," + str(dict_prot_id_loc_predictions[prot_id][loc_name]))
            fw.write("\n")
    fw.close()
